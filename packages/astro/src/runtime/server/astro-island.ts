...

{
	...
	const propTypes: PropTypeSelector = {
		0: (value) => maybeReviveObject(value),
	...
	const revive = (raw: any): any => {
		const [type, value] = raw;
		return type in propTypes ? propTypes[type](value) : undefined;
	};

	const reviveArray = (raw: any): any => (raw as Array<any>).map(revive);

	const maybeReviveObject = (raw: any): any => {
		if (typeof raw !== 'object') return raw;
		return Object.fromEntries(Object.entries(raw).map(([key, value]) => [key, revive(value)]));
	};
				...
				hydrate = async () => {
					...
					try {
						props = this.hasAttribute('props')
							? revive(JSON.parse(this.getAttribute('props')!))
							: {};
					} catch (e) {
